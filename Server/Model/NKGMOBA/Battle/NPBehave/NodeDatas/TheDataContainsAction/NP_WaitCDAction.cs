...
namespace ETModel
{
    ...
    public class NP_WaitCDAction: NP_ClassForStoreAction
    {
        ...
        public NPBehave.Action.Result WaitCDAction(bool hasDown)
        {
            ...
            if (!CDComponent.Instance.GetCDResult(this.Unitid, CDName.GetTheBBDataValue<string>()))
            {
                //Log.Info($"等待CD：{CDComponent.Instance.GetCDData(this.Unitid, CDName.GetTheBBDataValue<string>()).RemainCDLength}");
