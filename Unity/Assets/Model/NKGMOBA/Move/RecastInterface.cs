...
namespace ET
{
    ...
    public class RecastInterface
    {
        ...
        [DllImport(RecastDLL, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
        ...
        {
            {
                ...
                {
                    Log.Error(string.Concat("错误:", ($"Recast寻路 FindPath Error：- 起点非法 - spos:{spos} - MapId:{id}")));
                }
            ...
            }
            {
                ...
                {
                    Log.Error(string.Concat("错误:", ($"Recast寻路 FindPath Error：- 终点非法 - epos:{epos} - MapId:{id}")));
                }

                EPos = epos;
            }
            dtStatus status =
                (dtStatus) recast_findpath(id, new[] {-spos.x, spos.y, spos.z}, new[] {-epos.x, epos.y, epos.z});
            if (dtStatusFailed(status))
            {
                ...
                Log.Error(string.Concat("错误:", _strLastError));
                return false;
            }
        ...
        {
            dtStatus status = (dtStatus) recast_raycast(id, new float[] {-spos.x, spos.y, spos.z},
                new float[] {-epos.x, epos.y, epos.z});
            if (dtStatusFailed(status))
            {
                dtStatus statusDetail = status & dtStatus.DT_STATUS_DETAIL_MASK;
        ...
        public static float[] getHitPosition(int id)
        {
            unsafe
            {
                ...
                {
                    Log.Error(string.Concat("错误:", ($"RecstInterface getHitPosition Exception! - {e}")));
                    return null;
                }
            }
        }
        ...
        public static float[] fixposition(int id, Vector3 pos)
        {
            unsafe
            {
                try
                {
                    float[] posPoint = new float[3] {-pos.x, pos.y, pos.z};
                    IntPtr fixPos = recast_getfixposition(id, posPoint); // （pos.y+1）抬高一点计算
                ...
                {
                    Log.Error(string.Concat("错误:", ($"RecstInterface fixposition Exception! - {e}")));
                    return null;
                }
            }
        }
