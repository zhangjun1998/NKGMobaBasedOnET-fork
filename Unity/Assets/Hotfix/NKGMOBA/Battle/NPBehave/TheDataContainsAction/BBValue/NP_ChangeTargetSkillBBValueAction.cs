...
namespace ET
{
    ...
    {
        [LabelText("从这个数据配置中获取")] FromDataSet,

        [LabelText("从黑板中获取")] FromBBValue,
    }
    ...
    {
        [Tooltip(
            "比如诺手（Unit A）的Q技能会有一个碰撞体Unit（Unit B），这个Action是在UnitB上的一个行为树中的，如果此处为true，则会索引到UnitB而不是UnitA，否则就根据下面的<目标技能归属的英雄Unit Id>来获取目标")]
        [LabelText("目标技能归属Unit是否为自身的Unit")]
        public bool TargetUnitIsSelf = true;

        [HideIf("TargetUnitIsSelf")] [LabelText("目标技能归属的英雄Unit Id")]
        public NP_BlackBoardRelationData TargetUnitId = new NP_BlackBoardRelationData();

        [BoxGroup("目标技能Id配置")] [HideLabel] public VTD_Id TargetSkillId = new VTD_Id();

        [LabelText("要传递给目标技能的黑板值")]
        public NP_BlackBoardRelationData NPBBValue_ValueToChange = new NP_BlackBoardRelationData();

        [OnValueChanged("ApplyDataGetType")] public ValueGetType ValueGetType = ValueGetType.FromBBValue;
